...
| `platform`          | Yes      | One of `x`, `telegram`, `facebook`, `instagram`, `youtube`, `tiktok`, `bluesky`, `threads`, `linkedin`, `pinterest`, `mastodon`. The legacy alias `twitter` maps to `x`.            |
...
```json
{
  "id": "mastodon-main",
  "platform": "mastodon",
  "username": "yourname@example.social",
  "credentials": {
