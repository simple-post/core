...
Updated 2026-09-07 23:02 UTC. X testing was authorized and completed; Forem/DEV is now excluded at the user’s request. Application fixes belong to `core`; the live harness and local evidence belong to `core2`.

## Verified coverage

The current catalog has **501 supported scenario/interface combinations** across scheduler UI, MCP, and CLI-app, including X and excluding the 27 Forem/DEV cases and 3 paused YouTube playlist cases. **357 have verified evidence** for the configured test accounts. These are deduplicated results from saved runs across the observed deployments, not a claim that every case was rerun against the final commit. Unsupported combinations are excluded from the denominator; missing accounts, owner sessions, permissions, and provider limits remain incomplete. CLI-local is available in the harness but was not run because separate local provider credentials/aliases are not configured.
...
The thumbnail warning fix (`bdf54de`, production merge `5757b47`) preserves the successful video ID and returns a visible publishing-result warning with code `youtube_thumbnail_upload_failed` when the subsequent thumbnail upload fails. It passed 24 focused publisher tests plus SDK TypeScript, lint, and formatting checks. Both branches were pushed. The existing SSH connection verified that the container routed to `app.simplepost.social` runs production image revision `5757b47`, the deployed bundle contains `youtube_thumbnail_upload_failed`, and the public app returns HTTP 200. No extra live upload was performed to trigger the warning because the channel allowance is exhausted.

All changes above were committed and pushed to `main` and `release/prod`. Production behavior confirmed the YouTube readback routes, Threads mixed-media validation, and owner processing readback. The earlier YouTube scope expansion was subsequently removed and production verified the original approved scopes. On the final rollout, the advanced-page chunk changed to `page-223e49319367776e.js`; a normal Playwright upload showed the photo music, description, and cover controls. The saved TikTok UI cancellation regression then passed against production. The Coolify browser dashboard requires login; the September 7 thumbnail fix was verified independently over the existing SSH connection as recorded above.
...
The previous full offline harness run passed **428/428 tests**, TypeScript, and formatting. It targets the canonical application SDK inferred from the configured `/core/cli/bin/run.js`, rather than copying application code into `core2` or weakening contracts against its older SDK.

The installed-Chrome authentication helper and TikTok login-page detection additionally passed 12 focused offline tests. Authentication export preserves existing sessions on cancellation/failure and saves cookies, local storage, and IndexedDB with owner-only file permissions. Login redirects now stop verification immediately and retain the post receipt.
...
- **TikTok: 128 cases remain.** Refreshed creator capabilities confirm `canPost=true`, public/mutual-follow/self-only audiences, and enabled comment/duet/stitch capabilities. Manual sign-in through installed Chrome succeeded and a fresh owner session was saved on September 7. Reusing it in the headed automated Chrome observer still triggers a slider challenge; the next step is manual verification in that test window, not another login attempt. The September 7 verification-only retry of the earlier video reached a guest login page and found no exact marker. No new TikTok posts were submitted during this retry. All 15 nonpublishing cases were rechecked under the public-account configuration: seven CLI rejections (`live-20260906212116784-ae49c4c0`), six MCP rejections (`live-20260906212249346-5436680f`), and both cancellations (`live-20260906212304076-baa37971`). Both scheduled records were discarded, with no external posts. The harness now excludes MCP validation-only calls from posting budgets, while CLI cases that invoke publishing retain their conservative reservation. A single public-video probe (`live-20260906213331509-34c52f7a`, receipt `cmtqbwmsh000nmg3d690un1ei`, handle `v_pub_file~v2-1.7682537943286171670`) was accepted, but public observation encountered a slider CAPTCHA and no visible matching post. Its receipt is retained as inconclusive, not verified; do not republish it. PUBLIC video checks now use an empty browser session to prove public visibility when TikTok permits viewing, and challenge detection stops retries. Private, music, interaction, and inbox checks retain their separate observation requirements. Remaining settings still need genuine platform observation; inbox cases may require owner/mobile observation if the relevant UI is unavailable on the web.
- **YouTube: 23/39 verified; 16 remain; 3 playlist cases are explicitly paused.** The September 7 CLI-app batch added nine verified cases: video, video without caption, metadata, public/private/unlisted visibility, both made-for-kids values, and JSON input. The next smoke case (`live-20260907225651358-a729321c`) hit the channel upload allowance at 22:56 UTC and explicitly confirmed no video was created. All further uploads stopped. The remaining non-thumbnail cases are UI video-no-caption, schedule, draft-edit, all three privacy values, both made-for-kids values and metadata; MCP smoke, schedule and draft-edit; and CLI-app smoke.
- **YouTube custom thumbnails need channel verification.** The CLI-app thumbnail attempt (`live-20260907224816165-91d76a35`, receipt `cmtru0lqi00mao73d0kpir7lm`, video `euT1Q2t2tzs`) published its video, but the thumbnail did not match. Independent owner readback confirms `hasCustomThumbnail=false` after processing succeeded; the native Studio Settings → Channel → Feature eligibility page shows Intermediate features only **Eligible**, requiring phone verification. The user was asked to complete this in the Edmund Chrome profile. All three thumbnail cases remain incomplete. The existing video is retained and must not be reposted; the local journal is inconclusive with `youtube-owner-readback.json` attached. This is channel verification, not a new OAuth permission or Google app review. The existing `youtube.upload` scope supports [thumbnail uploads](https://developers.google.com/youtube/v3/docs/thumbnails/set).
- **YouTube quota recovery evidence is preserved.** Four older saved Google HTTP400 `uploadLimitExceeded` attempts were conclusively reconciled as not published through the normal account/post reconciliation API before fresh testing; evidence is in `.local/youtube-20260908/reconciliation.json`. The old and new CLI quota rejections retain their original process output and are recorded as failed, never passed. Playlist assignment remains disabled by main `c144e5f` / production `12d78e3`; no additional Google scopes or consent were requested.
...
```sh
yarn e2e:auth --chrome https://www.tiktok.com/@edmundclompton .local/auth/tiktok.json
yarn e2e:auth https://www.linkedin.com/feed/ .local/auth/linkedin.json
```

Run them one at a time and sign in normally. For `--chrome`, type `save` in the terminal after login; for the other command, resume Playwright Inspector. The saved sessions are discovered automatically. Successful session export does not guarantee TikTok permits automated observation.

TikTok documents a creator posting cap typically around 15 Direct Post submissions per 24 hours, so the full remaining matrix requires multiple daily batches. The current account offers public, friends-only, and self-only audiences; follower-only cases require a private account and remain blocked for this public account. Photo music, cover, interactions, inbox delivery, and commercial disclosure still need native observers before their live submissions can be verified. [TikTok posting limits](https://developers.tiktok.com/docs/en/content-sharing-guidelines).
