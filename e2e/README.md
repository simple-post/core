...
If login works in ordinary Chrome but fails in the Playwright window, try the installed-Chrome manual login mode:

```sh
yarn e2e:auth --chrome https://www.tiktok.com/@YOUR_HANDLE .local/auth/tiktok.json
```

This opens installed Google Chrome with a dedicated, persistent test profile beside the session file. Sign in yourself, keep the window open, then type `save` in the terminal only when your account is visibly logged in. Playwright attaches afterward through Chrome's local debugging connection to export cookies, local storage, and IndexedDB. It does not enter credentials or retry login. Cancel with Ctrl+C if login is blocked; the existing saved session is preserved. The dedicated Chrome window closes after export; your normal Chrome stays open.

Chrome requires a separate profile for remote debugging, so this does not copy your personal Chrome profile or its existing login. The new profile needs its own initial manual login. No browser identity spoofing or challenge bypass is used. A successful export is not proof that TikTok will accept the session in the automated verifier: public checks intentionally remain logged out, and private/settings checks may still encounter a challenge. Stop there and retain the existing post receipt. [Chrome debugging profile requirement](https://developer.chrome.com/blog/remote-debugging-port), [Playwright CDP connection](https://playwright.dev/docs/api/class-browsertype#browser-type-connect-over-cdp).
...
YouTube custom-thumbnail cases require channel phone verification: in YouTube Studio → Settings → Channel → Feature eligibility, Intermediate features must be **Enabled**, not merely **Eligible**. This is separate from OAuth consent; the existing `youtube.upload` scope supports thumbnail uploads. A channel upload-limit rejection stops further submissions until YouTube restores its allowance. If a video exists but its thumbnail failed, retain its receipt and inspect the existing video; never republish it automatically. [YouTube feature access](https://support.google.com/youtube/answer/9890437), [thumbnail API authorization](https://developers.google.com/youtube/v3/docs/thumbnails/set).
...
TikTok PUBLIC_TO_EVERYONE video visibility can be verified in a fresh logged-out browser, with the exact author, content, playable media, and permalink checks still required. This does not provide evidence for private audiences, music, or interaction settings. A TikTok CAPTCHA blocks verification and requires normal interactive browser access; the runner retains the receipt and never republishes it.
